        ...
        <motion.div
            ...
            <Form method="POST" >
                <div className=" shadow shadow-border border border-border dark:shadow-dark-border dark:border-dark-border flex flex-col gap-10 bg-background2 p-10 rounded dark:bg-dark-background2" >
                    <p>{mode}</p>
                    <p
                        className="text-red-400" >{actionData && actionData.message}</p>

                    <label htmlFor="email" />
                    <input
                        className="text-stone-800 p-2 rounded shadow-sm" id="email"
                        name="email"
                    ...
                    <button
                        ...
                        className="rounded bg-primary py-2 text-text shadow active:scale-95 duration-75 ease-linear cursor-pointer" >{buttonState ? (mode === "login"? "logging in..." : "signing up..."): mode}</button>
                    {mode === "login" ?
                        <p className="text-center" >Don't have an account , <span onClick={() => handleMode("signup")}
                            className="text-primary dark:text-dark-primary" ><Link>Sign up</Link></span></p> :
